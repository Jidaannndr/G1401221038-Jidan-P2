...
Data yang digunakan dalam kesempatan kali ini adalah data GDP Negara Spain periode tahun 1960-2023.

```{r}
#impor excel
library(readxl)
dtSpain<-read_excel("C:/Users/Jidan statistik/OneDrive/Documents/DATA MPDW PRAKTIKUM 2.xls")

```

# Eksplorasi Data

Sebelum melakukan regresi, akan diperlihatkan *plot time-series* dari data GDP Negara Spain periode tahun 1960-2023

```{r}
#Membentuk objek time series
data.ts<-ts(dtSpain$GDP, start = 1960, end = 2023, frequency = 1)
data.ts
...
```{r}
dt.gab <- cbind(aktual = c(data.ts,rep(NA,12)), 
                pemulusan1 = c(dt.sma,rep(NA,12)),
                pemulusan2 = c(dt.dma, rep(NA,12)),
                At = c(At, rep(NA,12)), 
                Bt = c(Bt,rep(NA,12)),
...
ts.plot(dt.gab[,1], xlab="Time Period ", ylab="GDP", 
        main= "DMA N=3 Data GDP", ylim=c(12000000000,2000000000000))
points(dt.gab[,1])
points(dt.gab[,3])
points(dt.gab[,6])
...
training<-dtSpain[1:52,2]
testing<-dtSpain[53:64,2]
...
plot(dtSpain, pch = 12, col = "darkblue",
     main = "Scatter Plot Tahun vs Nilai GDP",
     xlab = "Tahun",
     ylab = "Nilai GDP")

# Menampilkan Nilai Korelasi
cor(dtSpain$Tahun, dtSpain$GDP)
```

Berdasarkan scatter plot di atas, terlihat adanya hubungan / korelasi positif antara peubah tahun dengan nilai GDP, terlihat titik-titik pada plot yang naik ke arah kanan atas. Hal tersebut juga diperkuat dengan hasil perhitungan aplikasi `R` di mana didapatkan nilai korelasi sebesar $0.9466678$.
...
#model regresi
model<- lm(GDP~Tahun, data = dtSpain)
summary(model)
```

Model yang dihasilkan adalah $$y_i= -5.537e+13x_t$$ Berdasarkan ringkasan model dapat diketahui bahwa hasil uji F memiliki *p-value* \< $\alpha$ (5%). Artinya, minimal terdapat satu variabel yang berpengaruh nyata terhadap model. Hasil uji-t parsial kedua parameter regresi, yaitu intersep dan koefisien regresi juga menunjukkan hal yang sama, yaitu memiliki *p-value* \< $\alpha$ (5%) sehingga nyata dalam taraf 5%. Selanjutnya dapat dilihat juga nilai $R^2=0.8945$. Artinya, sebesar 89,45% keragaman nilai GDP dapat dijelaskan oleh peubah tahun. Hasil ini menunjukkan hasil yang bagus, seolah mendapatkan hasil terbaik. Namun, kita perlu melakukan uji terhadap sisaannya seperti berikut ini.
...
**Exact One-Sample Kolmogorov-Smirnov Test**:
   - **D = 0.12548**: Nilai statistik D mengukur sejauh mana distribusi data berbeda dari tabel kolmogorov smirnov D (dalam hal ini, distribusi normal).
   - **p-value = 0.2447**: Nilai p yang besar (umumnya > 0.05) juga menunjukkan bahwa tidak ada bukti kuat untuk menolak hipotesis nol (H₀) bahwa data mengikuti distribusi normal.
...
$$GDP= −(6.504223e^+13) + (3.293608e^+10) ×Tahun$$
Intercept (-6.504223e+13): Ini adalah konstanta, yang menunjukkan nilai GDP ketika Tahun adalah nol. Namun, dalam konteks ini, nilai intercept yang sangat besar secara negatif mungkin tidak memiliki interpretasi yang berarti, terutama jika Tahun tidak mungkin bernilai nol.

Koefisien Tahun (3.293608e+10): Ini menunjukkan bahwa untuk setiap peningkatan satu unit Tahun, GDP diprediksi meningkat sebesar 3.293608×10^10. nilai 3.293608×10^10 Ini menunjukkan tren peningkatan GDP seiring waktu.

2. Durbin-Watson Statistic
Original Durbin-Watson (0.18121, p-value: 1.009e-27):
Nilai statistik Durbin-Watson yang sangat rendah (0.18121) menunjukkan adanya autokorelasi positif yang kuat dalam residual. P-value yang sangat kecil (1.009e-27) menunjukkan bahwa autokorelasi ini sangat signifikan.
Transformed Durbin-Watson (1.59640, p-value: 3.926e-02):
Setelah melakukan transformasi (kemungkinan transformasi AR(1) atau sejenisnya), nilai Durbin-Watson meningkat menjadi 1.59640, yang lebih dekat ke nilai 2 (nilai yang menunjukkan tidak adanya autokorelasi). P-value dari transformasi ini (3.926e-02) menunjukkan bahwa autokorelasi telah berkurang secara signifikan, meskipun mungkin masih ada sedikit autokorelasi positif.
...
```
Rho (0.892002)
Nilai ini mengindikasikan tingkat autokorelasi dari model asli. Nilai rho sebesar 0.892002 menunjukkan adanya korelasi positif yang sangat kuat antara residual saat ini dan residual di masa lalu.
...
```{r}
#Transformasi Manual
GDP.trans<- dtSpain$GDP[-1]-dtSpain$GDP[-64]*rho
Tahun.trans<- dtSpain$Tahun[-1]-dtSpain$Tahun[-64]*rho
...
text(x=0.892, y=3.325508e+23, labels = "rho=0.892", cex = 0.8)
```
...
Setelah dilakukan tranformasi balik, didapatkan model dengan metode Hildreth-Lu sebagai berikut. $$y_i=-6.504204e^+13+32935983794x_t$$
...
Hasil uji Durbin-Watson juga menunjukkan bawah nilai DW sebesar $1.5964$ berada pada selang daerah tidak ada autokorelasi, yaitu pada rentang DU \< DW \< 4-DU atau $$1,5635 < DW < 1,6268$$. akan tetapi dapat dilihat dari *p-value* sebesar $0.03926$, di mana *p-value* \< $\alpha$=5%. Artinya tolak $H_0$ atau cukup bukti menyatakan bahwa ada autokorelasi dalam data nilai GDP dengan metode Hildreth-Lu pada taraf nyata 5%.
...
mseModelawal <- sseModelawal/length(dtSpain$GDP)
mseModelCO <- sseModelCO/length(dtSpain$GDP)
mseModelHL <- sseModelHL/length(dtSpain$GDP)
...
Berdasarkan hasil tersebut dapat diketahui bahwa hasil penanganan autokorelasi dengan metode Cochrane-Orcutt dan Hildreth-Lu memiliki SSE yang sama, sebesar $3.325508e+23$ dan lebih baik dibandingkan model awal ketika autokorelasi masih terjadi, yaitu sebesar $1.999806e+24$.
